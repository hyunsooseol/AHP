easyAHP=function(data){
  ...
  ConLambda=function(ConLn){
    Matrix10=matrix(rep(0,20*20),ncol=20)
    diag(Matrix10)=1
    Matrix10[1:n,1:n]=ConLn
    MarginMatrix10.1=margin.table(Matrix10,1)
    Iterations0=MarginMatrix10.1/20
    ...
    Iterations13=Matrix10%*%Scaling12
    Scaling13=Iterations12/max(Iterations13)
    
    Iterations14=Matrix10%*%Scaling13
    Scaling14=Iterations12/max(Iterations14)
    
    Iterations15=Matrix10%*%Scaling14
    Scaling15=Iterations12/max(Iterations15)
    
    Iterations16=Matrix10%*%Scaling15
    Scaling16=Iterations12/max(Iterations16)
    
    Iterations17=Matrix10%*%Scaling16
    Scaling17=Iterations12/max(Iterations17)
    
    Iterations18=Matrix10%*%Scaling17
    Scaling18=Iterations12/max(Iterations18)
    
    Iterations19=Matrix10%*%Scaling18
    Scaling19=Iterations12/max(Iterations19)
    
    Iterations20=Matrix10%*%Scaling19
    Scaling20=Iterations12/max(Iterations20)
    
     Iterations21=Matrix10%*%Scaling20
     Scaling21=Iterations12/max(Iterations21)
     
     Iterations22=Matrix10%*%Scaling21
     Scaling22=Iterations12/max(Iterations22)
     
   
    
    MatrixScaling=matrix(c(Scaling0,Scaling1,Scaling2,Scaling3,Scaling4,
                           Scaling5,Scaling6,Scaling7,Scaling8,Scaling9,
                           Scaling10,Scaling11,Scaling12,
                           Scaling13,Scaling14,Scaling15,Scaling16,Scaling17,
                           Scaling18,Scaling19,Scaling20,Scaling21,Scaling22),nrow=20)
    
    MarginMatrixScaling2=margin.table(MatrixScaling,2)
    Normal=MatrixScaling
    
    for(Normal.i in 1:23){
      Normal[,Normal.i]=MatrixScaling[,Normal.i]/MarginMatrixScaling2[Normal.i]
    }
    MarginMatrix10.2=margin.table(Matrix10,2)
    Eigenvalue=MarginMatrix10.2%*%Normal[,2:23]
    return(Eigenvalue[22])
  }
...
data=data.frame(maker1=c(1,2,3,4,5,7,8,9,10,11,
                         12,13,14,16,15,17,18,19,20,21),
                maker2=c(6,8,2,3,5,4,7,8,11,13,
                         10,15,19,9,21,20,18,14,16,17))
              
row.names(data)=c("item1","item2","item3","item4","item5",
